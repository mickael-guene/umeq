...
int neutralToX86Flags(int neutral_flags)
{
    int res = 0;
    int i;
...
int x86ToNeutralFlags(int x86_flags)
{
    int res = 0;
    int i;
...
int fcntl64_neutral(int fd, int cmd, uint32_t opt_p)
{
    int res = -EINVAL;
...
int syscall_neutral_32(Sysnum no, uint32_t p0, uint32_t p1, uint32_t p2, uint32_t p3, uint32_t p4, uint32_t p5)
{
    ...
    switch(no) {
        ...
        case PR_stat64:
            ...
            break;
        case PR_statfs64:
            res = syscall(SYS_statfs64, p0, p1, p2);
            break;
        case PR_sysinfo:
