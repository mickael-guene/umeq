...
guest_ptr *arm64_env_startup_pointer;

static pid_t gettid()
{
    return syscall_adapter_guest64(PR_gettid, 0, 0, 0, 0, 0, 0);
}
...
{
    guest_ptr *current = arm64_env_startup_pointer;
...
long arm64_openat(struct arm64_target *context)
{
    ...
    if (strcmp(pathname, "/proc/self/auxv") == 0) {
        ...
        res = syscall_adapter_guest64(PR_openat, context->regs.r[0], context->regs.r[1], context->regs.r[2],
