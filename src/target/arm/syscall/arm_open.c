...
static pid_t gettid()
{
    return syscall_adapter_guest32(PR_gettid, 0, 0, 0, 0, 0, 0);
}
...
int arm_open(struct arm_target *context)
{
    ...
    if (strcmp(pathname, "/proc/self/auxv") == 0)
        ...
        res = syscall_adapter_guest32(PR_open, context->regs.r[0], context->regs.r[1], context->regs.r[2],
                                               context->regs.r[3], context->regs.r[4], context->regs.r[5]);

    return res;
}

/* FIXME: also catch emulated path names */
int arm_openat(struct arm_target *context)
{
    int res;

    res = syscall_adapter_guest32(PR_openat, context->regs.r[0], context->regs.r[1], context->regs.r[2],
                                             context->regs.r[3], context->regs.r[4], context->regs.r[5]);
